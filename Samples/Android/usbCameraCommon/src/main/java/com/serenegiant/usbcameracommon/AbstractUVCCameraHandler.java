...
//					mUVCCamera.setPreviewSize(mWidth, mHeight, 1, 30, UVCCamera.DEFAULT_PREVIEW_MODE, mBandwidthFactor);
			...
			Log.d("handleStartPreview", "!!!!!! setPreviewDisplay!!!!");
			if (surface instanceof SurfaceHolder) {
				Log.d("SurfaceHolder", "SurfaceHolder");
				mUVCCamera.setPreviewDisplay((SurfaceHolder)surface);
			} if (surface instanceof Surface) {   //열화상 카메라는 surface 여기 부분으로 들어감!!
				Log.d("SurfaceHolder", "Surface");
				mUVCCamera.setPreviewDisplay((Surface)surface);  // setPreviewSize <-> setPreviewDisplay 사이즈를 비교 ....?
			} else {
				Log.d("SurfaceHolder", "SurfaceTexture");
...
//						Log.d("byte array ", Arrays.toString(stream.toByteArray()));
						os.flush();
