...
#define PREVIEW_PIXEL_BYTES_Y16 2
...
int UVCPreview::setPreviewSize(int width, int height, int min_fps, int max_fps, int mode, float bandwidth) {
	...
	if ((requestWidth != width) || (requestHeight != height) || (requestMode != mode)) {
		...
		LOGI("setPreviewSize params %d %d %d %d %d %d", width, height, min_fps, max_fps, mode, bandwidth);
		LOGI("------------------------requestMode !!! %d", requestMode);

		if (requestMode == UVC_FRAME_FORMAT_Y16) {
...
int UVCPreview::setFrameCallback(JNIEnv *env, jobject frame_callback_obj, int pixel_format) {
	...
	pthread_mutex_lock(&capture_mutex);
	{
		...
		if (frame_callback_obj) {
			...
			LOGI("setFrameCallback:mPixelFormat = %d", mPixelFormat);
...
void UVCPreview::callbackPixelFormatChanged() {
	...
	LOGI("callbackPixelFormatChanged %d", mPixelFormat);
	switch (mPixelFormat) {
	...
	if (!isRunning()) {
		...
		pthread_mutex_lock(&preview_mutex);
		{
			if (LIKELY(mPreviewWindow)) {
			  ...
			  //  param->arg1 = this;
			  //  int testParam = 1000;
			  //  param->arg2 = testParam;
			  //  int testParam2 = 2000;
			  //  param->arg3 = testParam2;
...
/*
    LOGI("broken frame!:format=%d,actual_bytes=%d/%d(%d,%d/%d,%d)",
...
//            LOGI("frame %x, %d", frame->data, frame->sequence);
        }
...
//
   // //TestParameter *param = reinterpret_cast<TestParameter *>(vptr_args);
   // TestParameter *param = (TestParameter*)vptr_args;
   // UVCPreview *preview = (UVCPreview *)param->arg1;
   // int number = param->arg2;
   // //JNIEnv env = param->arg3;
   // int number2 = param->arg3;
...
void UVCPreview::do_preview(uvc_stream_ctrl_t *ctrl) {
	...
	if (LIKELY(!result)) {
		...
		LOGI("Streaming...");
#endif
		LOGI("frameMode : %d", frameMode);
        if(frameMode == PIXEL_FORMAT_Y16) {
            LOGI("frameMode : PIXEL_FORMAT_Y16 %d", frameMode);
			...
			// MJPEG mode
			for ( ; LIKELY(isRunning()) ; ) {
					LOGI("Streaming... frameMode %d MJPEG", frameMode);
			...
			LOGI("yuvyv mode %d", LIKELY(isRunning()));
			for ( ; LIKELY(isRunning()) ; ) {
    		//	LOGI("for loop 진입");
				frame = waitPreviewFrame();
               // LOGI("Streaming... frameMode %d yuvyv", frameMode);
                //LOGI("width : %d, height : %d", frame->width, frame->height);

				if (LIKELY(frame)) {
				    //LOGI("frameMode : yuvyv mode %d",frameMode);
...
*/
				...
				//	LOGI("frame->data address : %X", frame->data);
