...
public class UVCCamera {
	...
	//damon added
	public static final int THERMAL_PREVIEW_WIDTH = 160;
	public static final int THERMAL_PREVIEW_HEIGHT = 120;

	public static final int THERMAL_PREVIEW_MODE = 12;
	public static final int VISUAL_PREVIEW_MODE = 1;
    ...
    public UVCCamera() {
    	...
    	Log.d("mNativePtr", String.format("%d", mNativePtr));
    	mSupportedSize = null;
	}
    ...
    public synchronized void open(final UsbControlBlock ctrlBlock) {
    	...
    	try {
			...
			Log.d("mNativePtr 2", String.format("%d %d %d %d %d %d %s", mNativePtr, mCtrlBlock.getVenderId(), mCtrlBlock.getProductId(), mCtrlBlock.getFileDescriptor(), mCtrlBlock.getBusNum(), mCtrlBlock.getDevNum(), getUSBFSName(mCtrlBlock)));
...
//    		mNativePtr = 0;	// nativeDestroyを呼ぶのでここでクリアしちゃダメ
	...
	public void setPreviewSize(final int width, final int height, final int min_fps, final int max_fps, final int frameFormat, final float bandwidthFactor) {
		Log.d("setPreviewSize params", String.format("%s, %s, %s, %s, %s, %s", width, height, min_fps, max_fps, frameFormat, bandwidthFactor));
		if ((width == 0) || (height == 0))
			throw new IllegalArgumentException("invalid preview size");
		if (mNativePtr != 0) {
			final int result = nativeSetPreviewSize(mNativePtr, width, height, min_fps, max_fps, frameFormat, bandwidthFactor);
			Log.d("result", String.format("%s", result));
			if (result != 0)
    ...
    public void setFrameCallback(final IFrameCallback callback, final int pixelFormat) {
    	Log.d("setFrameCallback", "callback!!!");
    	if (mNativePtr != 0) {
    ...
    public synchronized void startPreview() {
    	setFrameCallback(new IFrameCallback() {
			@Override
			public void onFrame(ByteBuffer frame) {
				Log.d("onFrame!!!", String.format("length: %d", frame.array().length));
				System.out.println("length:-------------------------------"+ frame.array().length);
				frame.order(ByteOrder.LITTLE_ENDIAN);
				Float[] list = new Float[100];
				for(int ii=0; ii<list.length; ii++){
					list[ii] = ((float)frame.getShort() / 100 - 273.15f);
					System.out.println("FrameCallback -----------------");
				}
				System.out.println("out of For {{ FrameCallback -----------------");
				Log.d("onFrame!!!", "onFrame!!! : " + Arrays.toString(list));
				Log.d("onFrame!", String.format("array length : %d", frame.array().length));
			}
		}, 12);
    	if (mCtrlBlock != null) {
			nativeStartPreview(mNativePtr);   // 멈췄을때 resume 해야 사네
			Log.d("nativeStartPreview", "nativeStartPreview : ");
    	}
    }
